...
`fmdb-udf-raha` 是面向 FMDB 和 Spark 的单元格级数据错误检测组件。工程参考 Raha 的弱监督错误检测思路，将字段画像、检测策略、稀疏特征、聚类采样、标签传播、列级模型、生产检测和 FMDB 平台治理整合为 Java 组件，并提供训练、检测、采样三个可注册到 Spark SQL 的异步表级 UDF。

工程不是带 `main` 方法的独立应用。它以组件库和 UDF Jar 的形式交付，由 FMDB 宿主进程负责组装 Spark 会话、共享仓储、权限检查、审计写入和后台任务消费者。
...
10. 通过 FMDB 表保存任务、模型、特征字典、检测结果和审计事件。
11. 提供权限控制、敏感值保护、保留清理、性能基准和生产就绪检查。
12. 通过三个 Spark SQL UDF 严格解析请求并异步创建幂等任务。
...
| FMDB 适配 | 已实现表或只读 SQL 加载、任务与结果写入、模型存储、审计和保留清理 |
...
| 层次 | 包 | 职责 |
| --- | --- | --- |
| 平台入口层 | `udf` | 注册函数、解析请求、权限审计、幂等建单和 JSON 应答 |
...
| 安全治理层 | `security`、`audit`、`retention` | 权限、审计、结果保护和过期清理 |
| 运行治理层 | `parallel`、`performance`、`production` | 并发控制、性能基准、容量建议和生产门禁 |
...
测试范围包括配置加载、数据加载、画像、策略、特征、聚类、采样、标签传播、训练、检测、模型生命周期、任务状态机、检查点、FMDB 适配、UDF、安全审计、保留清理、并行恢复和性能治理。
...
| `raha.security.result.mode` | `MASKED` | 结果值保护模式 |
| `raha.security.result.all-columns-sensitive` | `true` | 是否默认保护所有检测字段 |
| `raha.retention.intermediate-days` | `7` | 中间结果建议保留天数 |
| `raha.retention.detection-days` | `90` | 检测结果建议保留天数 |
...
| `PERMISSION_DENIED` | 调用方缺少任务、输入、结果、标注或模型权限 |
...
3. 注入生产权限检查器和审计服务。
4. 创建 `RepositoryBackedRahaUdfJobSubmitter`。
5. 调用 `new RahaUdfRegistrar().register(sparkSession, submitter)` 注册三个函数。
6. 启动后台任务消费者，将已创建任务交给训练、检测或采样服务执行。
7. 根据任务状态和 `resultLocation` 查询最终结果，不在 SQL UDF 调用中同步等待算法完成。
...
- `AllowAllRahaPermissionChecker`：跳过真实权限治理。
- `NoOpRahaAuditWriter`：不保存审计证据。
...
1. UDF 提交前检查任务提交、输入读取、结果写入以及标注或模型读取权限。
2. 权限采用默认拒绝模型，生产环境必须注入真实授权规则。
3. 结果值默认使用 `MASKED` 模式，并默认将所有检测字段视为敏感字段；也可配置 `HASH_ONLY`。
4. 任务提交、模型发布与回滚、检测结果读取和保留清理均支持审计事件。
5. 日志覆盖配置读取、Spark 或 FMDB 外部调用、核心阶段、耗时、关键分支和异常堆栈。
6. 日志和 UDF 错误响应不得写出原始单元格值、完整 SQL 或敏感参数。
7. FMDB 幂等写入通过业务键左反连接减少重复数据，但不构成跨进程原子事务，生产表仍应配置平台侧唯一约束或等价治理。

## 性能与资源建议

工程内置小型、中型、大型和宽表四类基准规格，并可根据行数、字段数和数据大小给出分区数、策略并发、字段并发、RVD 列对上限和缓存建议。

默认建议只是初始基线，必须在目标 FMDB 集群重新压测。需要特别关注以下 Driver 风险：

1. 部分策略会使用 `collectAsList` 收集候选。
2. 特征组装会按列收集值和频率结果。
3. 精确层次聚类计算复杂度较高，默认单列上限为 500 个样本。
4. 当前生产预测主路径仍包含 Driver 侧列表处理。
5. 默认性能探针主要观测 Driver 堆内存，执行器、磁盘和网络指标需要宿主扩展。

生产资源建议详见 `doc/20260715/Raha数据检测生产资源建议-202607151228.md`。
...
8. 保留清理依赖目标 FMDB 支持 `DELETE FROM` 及相应权限。
...
| `doc/20260715/Raha数据检测生产资源建议-202607151228.md` | 容量分档、资源配置和压测建议 |
| `doc/20260715/Raha数据检测迭代9落地与FMDB-UDF验收报告-202607151133.md` | FMDB 适配和 UDF 验收说明 |
| `doc/20260715/Raha数据检测迭代10落地与P2生产验收报告-202607151228.md` | 安全、审计、清理和生产门禁说明 |
