...
工程主体以组件库和 UDF Jar 的形式交付，由 FMDB 宿主进程负责组装 Spark 会话、共享仓储和后台任务消费者。`RahaContainerValidationApplication` 提供容器验收 `main` 方法，但不是通用生产服务启动器。
...
| Spark SQL UDF | 已支持程序化注册，以及 Hive 会话中通过 `ADD JAR` 和类名分别注册三个异步入口 |
| UDF 后台执行 | 已提供单次扫描的文件工作器，持续轮询、保活和告警仍需宿主平台实现 |
| 核心共享仓储 | 当前提供完整仓储契约和进程内实现，生产共享持久化需由宿主补充 |
| 安装脚本 | 已提供临时函数注册 SQL，尚未提供 FMDB 建表和一站式安装包 |
...
| Spark Hive | `provided`，仅 `ADD JAR` 加 `CREATE FUNCTION` 注册方式需要 |
...
| `scripts/register_raha_udfs.sql` | 通过 `ADD JAR` 分别创建三个临时函数 |
...
Spark SQL、Spark Hive、Spark MLlib 和 SLF4J 被声明为 `provided`，不会被打入完整 Jar，目标 FMDB 环境必须提供兼容版本。程序化注册不要求启用 Hive 会话；通过类名创建函数时必须提供 Spark Hive 3.3.1 兼容环境。
...
工程支持程序化注册和 `ADD JAR` 独立注册两种方式。两种方式创建的均为当前 Spark 会话中的函数，不会因为应用退出而继续存在。

#### 方式一：程序化注册

宿主创建可序列化的任务提交器，并在长期运行的 Spark 会话中调用：

```java
RahaUdfJobSubmitter submitter = new FileRahaUdfJobSubmitter(
        "/shared/raha/udf-requests");
new RahaUdfRegistrar().register(sparkSession, submitter);
```

注册器通过 `sparkSession.udf().register` 分别注册三个函数。任一函数注册失败时会清理运行时提交器并记录异常堆栈。

如果使用其他 `RahaUdfJobSubmitter` 实现，该实现必须能随 Spark UDF 安全序列化，或者保证函数只在持有该提交器的进程内执行。`RepositoryBackedRahaUdfJobSubmitter` 当前没有实现 Java 序列化接口，不应直接假设它可以跨 Driver 和 Executor 使用。

#### 方式二：ADD JAR 独立注册

该方式适用于启用了 Hive 支持的 Spark 3.3.1 会话。启动 Spark SQL 或宿主进程前，必须为 Driver 和 Executor 配置同一个共享任务目录：

```bash
spark-sql \
  --conf spark.sql.catalogImplementation=hive \
  --driver-java-options "-Draha.udf.queue-directory=/shared/raha/udf-requests" \
  --conf "spark.executor.extraJavaOptions=-Draha.udf.queue-directory=/shared/raha/udf-requests"
```
...
也可以执行 `scripts/register_raha_udfs.sql` 一次创建三个临时函数。脚本中的 Jar 路径必须先替换为目标环境实际路径。

独立注册的无参 UDF 会创建 `FileRahaUdfJobSubmitter`，将请求写为 `<幂等键>-<任务类型>.request`，并同时保存 `.receipt` 幂等回执。返回结果中的 `resultLocation` 是请求文件 URI，而不是算法最终结果位置。

文件提交模式要求 `idempotencyKey` 以英文字母或数字开头，后续只允许英文字母、数字、点、下划线和连字符，总长度不超过 128 个字符。该限制用于阻止目录穿越和非法文件名。

#### 后台消费要求

无论采用哪种注册方式，UDF 返回 `ACCEPTED` 都只表示异步请求已创建。宿主还必须：

1. 持续调用或封装 `FileRahaUdfJobWorker.runOnce()` 扫描共享目录。
2. 实现 `RahaUdfTaskDispatcher`，按任务类型调用采样、训练或检测服务。
3. 监控 `.failed` 文件并提供重试、告警和人工恢复。
4. 根据业务任务状态查询最终结果，不在 SQL UDF 调用中同步等待算法完成。

`FileRahaUdfJobWorker` 当前不是常驻进程，不会因为注册函数而自动启动。生产环境也不得使用 `InMemoryRahaRepository` 作为跨进程共享仓储。
...
3. 验证程序化注册和 `ADD JAR` 独立注册，以及合法请求、非法请求、重复提交和幂等冲突。
...
1. 当前文件任务工作器只提供单次扫描，不包含常驻循环和一站式生产装配器。
...
10. 当前只提供临时函数注册 SQL，没有自动创建 FMDB 平台表或永久函数的一站式安装包。
