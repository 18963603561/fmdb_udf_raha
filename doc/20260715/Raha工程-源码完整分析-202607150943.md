...
文档首次生成于 2026-07-15 09:43，本次于 2026-07-17 10:20 基于提交 `86f1fc9` 及其后的当前工作区重新分析。与上一版文档对应的源码快照相比，工程从“包含部分生产治理能力的组件”收缩为“Raha 核心算法、FMDB 适配和集群验证组件”。
...
当前工作区还扩充了 `RahaUdfRequestParser.ALLOWED_KEYS` 的中文源码说明，逐项记录八个通用字段和三个任务专属字段的必填性、用途与样例。该变化只增强源码可读性，没有修改字段白名单、解析规则或 UDF 运行契约。
...
本次在 JDK 8u492 和 Maven 3.9.12 环境执行 `mvn -q clean test`，Surefire 汇总为 136 项测试全部通过，0 失败、0 错误、0 跳过。测试日志中出现的模拟阶段失败、聚类失败和非法 UDF 请求异常均为故障隔离测试的预期分支。随后执行 `mvn -q -DskipTests verify`，编译、打包和 Java 8 API 兼容性检查均通过。当前工作区扩充请求字段说明后，又定向执行 `RahaTableUdfIntegrationTest` 和 `RahaUdfConfigurationTest`，6 项测试全部通过。
