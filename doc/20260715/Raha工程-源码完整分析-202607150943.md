...
文档首次生成于 2026-07-15 09:43，本次按 2026-07-15 14:46 的当前工作区源码重新分析，包含分析过程中出现的未提交模型训练与配置调整。快照规模如下：
...
13. 注册训练、检测、采样三个表级 UDF，严格解析请求并异步创建幂等任务。
14. 通过 FMDB 表保存任务状态、模型参数、特征字典、检测结果和审计事件。
15. 对任务提交、模型生命周期、结果访问和过期清理执行权限控制与审计。
16. 从统一配置文件生成算法、安全、UDF、保留和性能配置。
17. 生成性能基准数据、采集阶段指标、给出容量分档建议并执行生产就绪检查。

一句话概括：该工程已经从 Raha 算法库演进为包含 FMDB 接入、表级 UDF、安全审计、运行治理和生产验收辅助能力的数据质量组件。
...
工程没有启动类和依赖注入容器，仍需由 FMDB 注册进程组装 Spark 会话、仓储、权限适配器、审计写入器和后台任务执行器。`RahaConfigFactory` 已提供统一配置工厂，`RahaUdfRegistrar` 已提供 UDF 注册入口，但 UDF 本身只负责异步建单，不负责在函数调用线程内执行完整算法任务。
...
n02 --> n04[异步任务 job repository];
n04 --> n05[后台业务服务 service];
n05 --> n06[策略与特征 strategy feature];
n05 --> n07[聚类与标注 cluster sampling label];
n05 --> n08[训练与检测 model detection evaluation];
n06 --> n09[领域对象 data checkpoint];
n07 --> n09;
n08 --> n09;
n06 --> n10[统一仓储 repository];
n07 --> n10;
n08 --> n10;
n05 --> n11[并行控制 parallel];
n05 --> n12[FMDB 适配 fmdb];
n12 --> n13[FMDB 表与目录];
n03 --> n12;
n14[统一配置 config] --> n02;
n14 --> n05;
n14 --> n15[性能治理 performance];
n15 --> n16[生产门禁 production];
n17[保留清理 retention] --> n03;
n17 --> n12;
```
...
| 层次    | 包                                                                                  | 职责                    |
| ----- | ---------------------------------------------------------------------------------- | --------------------- |
| 平台入口层 | `udf`                                                                              | 注册三个表级函数，解析请求并提交异步任务  |
...
当前 30 个主代码包及 Java 文件数量如下，合计 308 个文件：

| 包                                     | 文件数 |
| ------------------------------------- | ---:|
| `com.fiberhome.ml.raha.audit`         | 8   |
| `com.fiberhome.ml.raha.checkpoint`    | 6   |
| `com.fiberhome.ml.raha.cluster`       | 10  |
...
| `com.fiberhome.ml.raha.evaluation`    | 8   |
| `com.fiberhome.ml.raha.feature`       | 8   |
| `com.fiberhome.ml.raha.fmdb`          | 9   |
| `com.fiberhome.ml.raha.job`           | 23  |
| `com.fiberhome.ml.raha.label`         | 10  |
| `com.fiberhome.ml.raha.model`         | 24  |
