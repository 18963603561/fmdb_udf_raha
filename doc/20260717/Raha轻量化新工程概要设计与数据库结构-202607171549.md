...
`rowKeyColumns` 不是“需要采样的列”。采样单位始终是一整行，策略、特征、标注和检测默认处理全部输入列，不再提供目标列选择参数。业务键只用于精确定位具体数据行。
...
| 对象 | 主要字段 |
| --- | --- |
| `SampleResult` | 采样批次、请求预算、实际元组数、结果位置、耗时 |
| `TrainResult` | 训练模式、父模型集合、新模型集合、样本数、成功列、跳过列和耗时 |
| `DetectResult` | 检测批次、检查单元格数、疑似错误数、结果位置、耗时 |
...
首期训练固定从 `raha_cell_label` 读取所选采样批次的直接标签，不允许请求任意结构的标注表。外部标注系统应通过适配器写入标准标签表。

`datasetId`、`inputReference`、`sourceType`、行身份规则和 `snapshotId` 全部从 `raha_sample_batch` 读取，不再由训练请求重复传入。`trainingMode` 不作为请求字段：`baseModelSetVersion` 为空时自动生成 `FULL`，非空时自动生成 `INCREMENTAL`。

一次训练支持多个采样批次，但这些批次必须属于同一数据集、同一快照、同一输入来源、相同模式和相同行标识规则。标签按单元格合并；重复且一致的标签去重，冲突标签使训练失败。
...
请求指纹、采样批次标识、模型集合版本、列模型版本、检测批次标识、记录创建时间和 ORC 分区日期都由服务内部生成，不属于请求参数。相同的已解析请求和输入内容生成相同指纹及输出标识。
...
2. 策略计划由输入画像、算法配置和实现版本确定。
3. 同一请求、同一快照、同一随机种子必须生成相同采样顺序。
4. 聚类在每列独立执行，不跨列比较单元格。
5. 元组得分由其覆盖的低覆盖聚类数量、列权重和随机稳定打散值组成。
6. 同一个行标识在一个采样批次中最多出现一次。
7. 采样批次只负责选择元组，不预填人工标签，也不要求修复值。
8. 采样明细写完后再写采样批次头记录。
...
3. 校验数据集、字段模式和值规范化版本兼容。
4. 对本次训练快照执行父模型冻结的策略计划。
5. 严格使用父模型特征字典生成本次新增样本特征。
6. 对本次直接标签执行列内传播并生成新增训练样本。
7. 按数据集、行标识和字段合并父样本与新增样本，同一业务单元格以新样本为准。
8. 对每列合并后的完整样本集重新训练逻辑回归。
9. 保存合并后的训练样本快照和新列模型。
10. 创建引用父模型集合的新模型集合版本。

增量训练保持父模型的策略计划、特征字典和值规范化版本不变。新数据中出现但父字典不存在的特征不会进入模型。需要新增策略或扩展特征空间时必须执行全量训练。
...
4. 最后写 `raha_model_set`，同时保存采样批次列表和冻结策略计划。
...
1. 模型覆盖的目标字段必须存在且类型兼容。
2. `RVD` 策略依赖的关联字段必须存在且类型兼容。
3. 输入可以包含模型未覆盖的额外字段，额外字段默认不检测。
...
默认只写 `score >= threshold` 的疑似错误单元格。评测场景可通过配置写全部预测结果。
...
| 表名 | 用途 | 逻辑主键 |
| --- | --- | --- |
| `raha_sample_batch` | 已提交采样批次头 | `sample_batch_id` |
| `raha_sample_tuple` | 采样批次选择的元组 | `sample_batch_id, row_id` |
| `raha_cell_label` | 人工直接单元格标签 | `sample_batch_id, row_id, column_name` |
| `raha_model_set` | 已完整提交的模型集合头 | `model_set_version` |
| `raha_column_model` | 模型集合内列模型 | `model_set_version, column_name` |
| `raha_training_example` | 模型实际使用的训练样本 | `model_set_version, column_name, snapshot_id, row_id` |
| `raha_detection_batch` | 已提交检测批次头 | `detection_batch_id` |
| `raha_detection_result` | 单元格检测结果 | `detection_batch_id, row_id, column_name` |
...
2. 训练服务读取全部 `raha_sample_batch` 并校验数据集、快照、模式和行标识规则一致。
3. 通过 `sample_batch_id` 读取并合并 `raha_cell_label`。
4. 增量训练额外读取父 `raha_model_set`、`raha_column_model` 和 `raha_training_example`。
5. 训练完成后写入新模型集合对应的训练样本和列模型；训练样本保留可确定的来源采样批次，列模型记录同时包含该列特征字典。
6. 最后写 `raha_model_set`，其中包含全部采样批次标识和冻结策略计划，使新模型集合可被检测读取。
...
1. 检测请求通过 `modelSetVersion` 读取一个 `raha_model_set`。
2. 从模型集合读取冻结策略计划，再按模型集合读取多条列模型及其特征字典。
3. 检测服务读取输入数据，执行冻结策略、兼容特征和列模型预测。
4. 先写 `raha_detection_result`，再写 `raha_detection_batch` 头记录。
...
| `raha_sample_batch` | `raha_cell_label` | `sample_batch_id` | 一对多 |
| `raha_sample_batch` | `raha_model_set` | `sample_batch_ids_json` | 多对多引用 |
| `raha_model_set` | `raha_column_model` | `model_set_version` | 一对多 |
| `raha_model_set` | `raha_training_example` | `model_set_version` | 一对多 |
| `raha_model_set` | `raha_model_set` | `parent_model_set_version` | 父子版本 |
...
`sample_batch_id` 由 `request_fingerprint` 确定性生成。相同请求和相同输入内容返回同一批次。

### 14.2 `raha_sample_tuple`
...
`FULL` 模式的父版本必须为空；`INCREMENTAL` 模式的父版本必须存在。采样批次通常数量较少，直接保存在 `sample_batch_ids_json` 中，不再建立关联表。若策略计划超过 FMDB 单行大小限制，可以把压缩后的计划作为模型载荷保存并在该字段记录载荷地址，不重新拆回策略计划表。

### 14.5 `raha_column_model`
...
`detection_batch_id` 由 `request_fingerprint` 确定性生成。不保存运行中、失败或重试次数。

### 14.8 `raha_detection_result`
...
| `raha_strategy_plan` | 合并为 `raha_model_set.strategy_plan_json` | `{"targetColumns":["customer_phone"]}` |
| `raha_feature_dictionary` | 合并为 `raha_column_model.feature_dictionary_json` | `dict_customer_phone_001` |
| `raha_model_sample_batch` | 合并为 `raha_model_set.sample_batch_ids_json` | `["sample_orders_20260717_001","sample_orders_20260717_002"]` |
...
八张业务表使用 ORC 格式并分别建表。它们的字段结构、访问频率和保留周期不同，不合并为一张通用宽表，也不使用大段通用 JSON 载荷模拟不同记录类型。

表英文名适合作为存储根目录下的一级目录，不作为每张独立表的分区字段。独立表中的表名是恒定值，把它再次写成分区列不能产生分区裁剪收益。推荐目录形式如下：

```text
/raha/raha_sample_batch/
/raha/raha_sample_tuple/partition_date=2026-07-17/
/raha/raha_detection_result/partition_date=2026-07-18/
```
...
| --- | --- | --- | --- |
| `raha_sample_batch` | 首期不分区 | 不适用 | `sample_batch_id` |
| `raha_sample_tuple` | `partition_date` | 采样批次创建时间 | `sample_batch_id, row_id` |
| `raha_cell_label` | `partition_date` | 采样批次创建时间 | `sample_batch_id, column_name, row_id` |
| `raha_model_set` | 首期不分区 | 不适用 | `model_set_version` |
| `raha_column_model` | 首期不分区 | 不适用 | `model_set_version, column_name` |
| `raha_training_example` | `partition_date` | 模型集合创建时间 | `model_set_version, column_name, row_id` |
| `raha_detection_batch` | 首期不分区 | 不适用 | `detection_batch_id` |
| `raha_detection_result` | `partition_date` | 检测批次创建时间 | `detection_batch_id, column_name, row_id` |
...
2. 八张标准 ORC 表最终所在数据库、具体建表语法以及 FMDB 是否允许头表不分区。
