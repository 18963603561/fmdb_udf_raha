...
| 节点 | 级别 | 摘要 |
| --- | --- | --- |
| 用例开始和成功结束 | `info` | 输入标识、目标字段、结果数量和耗时 |
...
- 采样未指定 `targetColumns` 时展开全部可检测字段，显式指定时只处理目标字段。
- 训练未指定 `targetColumns` 时使用有有效标签的采样目标字段，显式字段必须属于采样目标并集。
- 检测未指定 `targetColumns` 时使用模型集合全部字段，显式字段必须是模型字段子集。
- 默认全部与显式完整字段列表生成相同的计算范围和业务结果。
...
- 增量训练复用父策略计划和特征字典。
- 增量训练指定部分目标字段时只重训目标字段，并保留其他父列模型。
- 模型兼容校验和冻结特征映射。

### 20.3 数据库适配测试

- 八张 `dw.raha_*` ORC 表的字段名、类型和存储位置严格匹配。
- `target_columns_json`、`model_columns_json` 和 `trained_columns_json` 保存展开后的确定字段列表。
- 每张明细表的逻辑主键可防止重复记录。
- 相同请求指纹返回既有结果。
- 输出标识相同但请求指纹不同时报冲突。
- 没有头记录的明细不会被业务加载。
...
10. 分别验证采样、训练和检测默认全部字段与显式字段子集。
11. 验证增量训练只重训指定字段并保留未选中的父列模型。
...
- 建立请求、结果、异常和配置对象。
- 建立 `dw` 库八张标准 ORC 表、`/fmdb/raha/` 目录及 FMDB 表网关测试。
- 实现三个请求的 `targetColumns` 默认展开、显式校验和指纹计算。
...
- 采样、训练和检测均支持 `targetColumns`，未指定时按阶段规则处理全部可用字段。
- 增量训练指定部分字段时只重新训练目标字段，并保留其他父列模型。
...
- 八张 `dw.raha_*` ORC 表均通过模式、存储位置和逻辑主键测试。
- ORC 存储目录统一位于 `/fmdb/raha/`。
- 模型集合和批次头均按最后提交规则写入。
- 直接标签可检测过期值和冲突。
- 相同请求指纹不会产生重复业务产物。
