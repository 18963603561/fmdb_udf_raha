INSERT INTO TABLE dw.raha_annotation_record
...
SELECT
  ...
  to_json(named_struct(
    ...
    'errorColumns', array_remove(array(
      IF(name_error = 1, 'name', CAST(NULL AS STRING)),
      IF(age_error = 1, 'age', CAST(NULL AS STRING)),
      IF(phone_error = 1, 'phone', CAST(NULL AS STRING)),
      IF(id_error = 1, 'id_card', CAST(NULL AS STRING)),
      IF(email_error = 1, 'email', CAST(NULL AS STRING)),
      IF(address_error = 1, 'home_address', CAST(NULL AS STRING))
    ), CAST(NULL AS STRING)),
