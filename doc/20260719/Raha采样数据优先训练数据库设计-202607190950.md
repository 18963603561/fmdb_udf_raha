...
最终物理表模式总数为九张：七张业务物理表加两张运行控制表。`raha_job_run` 默认保存，用于幂等、状态、零结果和失败审计；`raha_job_stage_attempt` 只用于跨执行阶段恢复和检查点审计，默认关闭入库，需要断点恢复时再开启。关闭表入库不会从默认建表脚本中删除表结构。
...
| 配置键 | 默认值 | 对应物理表 | 关闭后的行为 |
| --- | --- | --- | --- |
| `raha.persistence.table.sample-record.enabled` | `true` | `dw.raha_sample_record` | 不保存采样行，不能生成可恢复的人工标注任务 |
| `raha.persistence.table.annotation-record.enabled` | `true` | `dw.raha_annotation_record` | 不保存用户标注；该数据不可重新计算，不建议关闭 |
| `raha.persistence.table.training-column-artifact.enabled` | `true` | `dw.raha_training_column_artifact` | 不保存列画像、策略计划、特征字典和列级摘要 |
| `raha.persistence.table.training-cell.enabled` | `false` | `dw.raha_training_cell` | 特征、聚类和传播明细只在当前任务使用，失败后重新计算 |
| `raha.persistence.table.training-example.enabled` | `true` | `dw.raha_training_example` | 不冻结实际入模样本，模型训练完成后无法按原样复现 |
| `raha.persistence.table.model-artifact.enabled` | `true` | `dw.raha_model_artifact` | 模型只保留在当前存储实例缓存，进程重启后无法加载 |
| `raha.persistence.table.detection-result.enabled` | `true` | `dw.raha_detection_result` | 错误结果只即时返回，不能后续查询、导出和审计 |
| `raha.persistence.table.job-run.enabled` | `true` | `dw.raha_job_run` | 不保存任务状态、零结果和失败信息，只适合同步无状态调用 |
| `raha.persistence.table.job-stage-attempt.enabled` | `false` | `dw.raha_job_stage_attempt` | 不保存阶段尝试和检查点，跨执行失败时整任务重跑 |

`dw.raha_training_column_artifact` 内五类 JSON 产物继续独立控制：

| 配置键 | 默认值 | 对应字段 | 关闭后的行为 |
| --- | --- | --- | --- |
| `raha.persistence.column-artifact.profile.enabled` | `true` | `profile_json` | 重复训练时重新执行 `ColumnProfiler` |
| `raha.persistence.column-artifact.strategy-plan.enabled` | `true` | `strategy_plan_json` | 不保存策略计划和版本，不能用于已发布模型的解释和一致性校验 |
| `raha.persistence.column-artifact.feature-dictionary.enabled` | `true` | `feature_dictionary_json` | 不保存特征编号定义，模型不能跨进程预测 |
| `raha.persistence.column-artifact.cluster-summary.enabled` | `false` | `cluster_summary_json` | 聚类摘要只记录到当前任务日志，可按需重新计算 |
| `raha.persistence.column-artifact.propagation-summary.enabled` | `false` | `propagation_summary_json` | 标签传播摘要不入库，可按需重新计算 |

代码包和持久化目标的对应关系：

| 代码包或阶段 | 主要持久化目标 | 是否属于可放弃的中间产物 |
| --- | --- | --- |
| `sampling` | `sample-record` | 否，人工标注和 c1 合并依赖 |
| `data.profile` | 列级 `profile` | 是，可重新画像但计算成本较高 |
| `strategy` | 列级 `strategy-plan` | 计划正文不可放弃，执行临时状态不入库 |
| `feature` | `training-cell`、列级 `feature-dictionary` | 单元格向量可放弃，特征字典不可放弃 |
| `cluster` | `training-cell`、列级 `cluster-summary` | 是，当前任务完成后可重新聚类 |
| `label.propagation` | `training-cell`、列级 `propagation-summary` | 传播过程可放弃，最终直接标注不能放弃 |
| `model` | `training-example`、`model-artifact` | 训练样本可按风险关闭，跨任务模型不可放弃 |
| `detection` | `detection-result` | 是，调用方即时消费结果时可关闭 |
| `job`、`repository` | `job-run`、`job-stage-attempt` | 阶段尝试可放弃，任务状态默认保留 |

配置判断顺序：

1. `raha.persistence.enabled=false` 优先级最高，不初始化默认表，也不向标准九表写入数据。
2. 总开关开启后，先判断目标物理表开关；表开关关闭时，该表所有产物都只在当前任务上下文使用。
3. 写 `dw.raha_training_column_artifact` 时再判断五类列级产物开关，关闭的 JSON 字段写空值，不影响当前任务计算。
4. 关闭入库只跳过 FMDB 查询和写入，不跳过画像、策略、特征、聚类、标签传播、训练或预测计算。
5. 专用写入器先判断目标开关，生产 `SparkSqlFmdbTableGateway` 再按标准表名兜底判断，防止后续写入器绕过配置。
6. 自定义结果表不属于九张标准物理表，不受表级开关自动拦截，调用方必须显式决定是否写入。

启动时必须校验以下依赖关系：

| 已开启产物 | 必须同时开启 |
| --- | --- |
| `annotation-record` | `sample-record` |
| `training-cell` | `training-column-artifact`、`feature-dictionary` |
| `training-example` | `training-column-artifact`、`feature-dictionary` |
| `model-artifact` | `training-column-artifact`、`strategy-plan`、`feature-dictionary` |
| `job-stage-attempt` | `job-run` |

依赖不满足时，`FmdbPersistenceConfig` 在装配阶段直接抛出配置异常，不允许启动后写出无法加载或无法关联的半成品。
...
raha.persistence.table.sample-record.enabled=true
raha.persistence.table.annotation-record.enabled=true
raha.persistence.table.training-column-artifact.enabled=true
raha.persistence.table.training-cell.enabled=false
raha.persistence.table.training-example.enabled=true
raha.persistence.table.model-artifact.enabled=true
raha.persistence.table.detection-result.enabled=true
raha.persistence.table.job-run.enabled=true
raha.persistence.table.job-stage-attempt.enabled=false

raha.persistence.column-artifact.profile.enabled=true
raha.persistence.column-artifact.strategy-plan.enabled=true
raha.persistence.column-artifact.feature-dictionary.enabled=true
raha.persistence.column-artifact.cluster-summary.enabled=false
raha.persistence.column-artifact.propagation-summary.enabled=false
```

需要标签传播重试和阶段断点恢复时，只增加以下覆盖配置：

```properties
raha.persistence.table.training-cell.enabled=true
raha.persistence.table.job-stage-attempt.enabled=true
raha.persistence.column-artifact.cluster-summary.enabled=true
raha.persistence.column-artifact.propagation-summary.enabled=true
```

任务算法配置不再承担物理表持久化职责，所有写入决策统一由 `FmdbPersistenceConfig.shouldPersist` 按明确的物理表或列级产物枚举执行。
...
3. 九张表的独立开关只控制入库，不裁剪建表脚本；自动建表开启时仍幂等创建完整九表模式，避免不同实例形成不同表结构。
4. SQL 资源不存在、语句解析失败或 Spark SQL 执行失败时终止初始化。
5. 异常日志必须记录脚本资源和语句序号，但不能输出敏感业务数据。
6. 自动建表只解决“表不存在”，不会自动修改已有表结构。
