...
2. 本工程可以使用本地 `lib` 包中的 Spark、FMDB 和公共依赖 jar，但不依赖旧工程源码、旧 UDF 类或旧命令类。
3. `lib2.12` 不纳入本方案分析，也不作为 POM、编译 classpath 或运行 classpath 的依据。
...
当前新工程可以使用 `lib` 包。建议把可用 jar 统一放到本工程 `lib/` 目录，或在开发期从 `F:\ai-code\dw_fmdb_udf\lib` 引用。

重要约束：

1. 只使用 `lib`，不关注 `lib2.12`。
2. 只把 jar 当作依赖，不把旧工程 Java 或 Scala 源码作为实现基础。
3. Spark 主线锁定 Spark 3.3.1、Scala 2.12、JDK 1.8。
4. `lib` 中存在 Spark 4.0.0 的 `_2.13` jar，但当前方案不使用。
5. 若使用 Spark MLlib，需要确认运行环境或本工程 `lib` 中存在 `spark-mllib_2.12-3.3.1.jar`。

### 4.2 当前可见 jar 判断

| 类型 | jar | 判断 |
| --- | --- | --- |
| Spark 3 SQL | `spark-sql_2.12-3.3.1.jar` | 当前主线 |
| Spark 3 Catalyst | `spark-catalyst_2.12-3.3.1.jar` | 当前主线 |
| Spark 4 SQL | `spark-sql_2.13-4.0.0.jar` | 不用于当前方案 |
| Spark 4 Catalyst | `spark-catalyst_2.13-4.0.0.jar` | 不用于当前方案 |
| FMDB 增强 | `spark-enhance-2.6.0-SNAPSHOT.jar` | 可作为 jar 能力参考 |
| FMDB 增强依赖 | `spark-enhance-depends-2.6.0-SNAPSHOT.jar` | 可作为 jar 能力参考 |
| FMDB 扩展 | `spark-extensions-2.6.0-SNAPSHOT.jar` | 与扩展函数包可能有重复类 |
| SQL 公共包 | `sql-common-2.3.0-SNAPSHOT.jar` | 可用 |
| SQL Spark 包 | `sql-spark-2.3.0-SNAPSHOT.jar` | 可用 |
| SQL 扩展函数 | `sql-extended-functions-2.3.0-SNAPSHOT.jar` | 可用 |
| GDK | `gdk-db3-3.7.0.jar` | 可用 |
| 元数据知识包 | `meta-know-V0.2.88.jar` | 可用 |

注意：当前本地可见清单中没有确认到 `spark-mllib_2.12-3.3.1.jar`。如果运行平台已经提供 MLlib，可在 POM 中使用 `provided`；如果平台未提供，需要补齐同版本 jar。

### 4.3 依赖使用原则

| 原则 | 说明 |
| --- | --- |
| Spark 版本一致 | `spark-sql`、`spark-catalyst`、`spark-mllib` 必须同为 `3.3.1` |
| Scala 后缀一致 | 当前 Java 8 主线使用 `_2.12`，不能混用 `_2.13` |
| FMDB jar 二选一 | `spark-extensions` 和 `sql-extended-functions` 若有重复类，不应同时显式引入 |
| 平台依赖优先 `provided` | 运行环境已提供的 Spark 和 Hive 依赖避免打入业务包 |
| 本地 jar 集中管理 | 新工程统一使用 `${project.basedir}/lib`，避免引用多个外部路径 |
...
<properties>
    ...
    <local.lib.dir>${project.basedir}/lib</local.lib.dir>
</properties>
```
...
如果平台不提供 MLlib，则需要把同版本 `spark-mllib_2.12-3.3.1.jar` 补齐到 `lib` 或使用规则加权融合兜底。

FMDB 本地 jar 可用 `system` 作用域临时接入，但最终建议由平台依赖管理统一提供。
