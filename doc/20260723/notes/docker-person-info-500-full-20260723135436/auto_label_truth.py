...
def load_truth(path):
    ...
    return {row["key"]: row["errorColumns"] for row in payload["rows"]}
...
def auto_label(input_path, truth_path, output_path, summary_path):
    ...
    for row_index in range(1, sheet.nrows):
        values = {
            column: cell_text(sheet.cell_value(row_index, indexes[column]))
        ...
        write_sheet.write(row_index, indexes["_row_label"], 1 if is_error else 0)
        write_sheet.write(row_index, indexes["_error_columns"], ",".join(error_columns))
        write_sheet.write(
            row_index,
            indexes["_comment"],
...
def main():
    parser = argparse.ArgumentParser(description="按真值文件生成 person_info 标注 Excel")
