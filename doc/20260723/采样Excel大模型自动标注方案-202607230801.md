...
| 配置项 | 默认值 | 说明 |
| --- | --- | --- |
| `autoLabelContextWindowTokens` | 按模型识别 | 模型上下文窗口，私有部署可覆盖 |
| `autoLabelMaxRowsPerBatch` | 按容量计算 | 单次调用最大行数 |
| `autoLabelMaxCharsPerBatch` | 按容量计算 | 单次请求最大输入字符数 |
| `autoLabelMaxColumnsPerBatch` | 按容量计算 | 宽表按列分批阈值 |
...
| `autoLabelContextWindowTokens` | 按模型识别 | 模型上下文窗口令牌数 |
| `autoLabelMaxRowsPerBatch` | 按容量计算 | 单批最大行数 |
| `autoLabelMaxCharsPerBatch` | 按容量计算 | 单批最大输入字符数 |
| `autoLabelMaxColumnsPerBatch` | 按容量计算 | 单批最大字段数 |
